...
const contactFieldDefs = [
  ...
  { key: "Horario", label: "Horario", placeholder: "Lun - Sab: 8:00 AM - 7:00 PM\nDom: 9:00 AM - 2:00 PM", multiline: true },
]
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-foreground/40 px-6">
      <div className="w-full max-w-sm border border-border bg-card p-8">
        ...
        <div className="flex gap-3">
          <button onClick={onConfirm} className="flex-1 rounded-sm bg-destructive py-2.5 text-sm font-medium text-destructive-foreground hover:bg-destructive/90">Eliminar</button>
          <button onClick={onCancel} className="flex-1 rounded-sm border border-border bg-transparent py-2.5 text-sm font-medium text-foreground hover:bg-muted">Cancelar</button>
        </div>
      </div>
    </div>
  )
...
function Thumb({ src, alt }: { src: string | null; alt: string }) {
  return (
    <div className="relative h-10 w-10 overflow-hidden rounded-sm bg-muted">
      {src ? <Image src={src} alt={alt} fill className="object-cover" /> : <div className="flex h-full w-full items-center justify-center"><ImageIcon className="h-4 w-4 text-muted-foreground" /></div>}
    </div>
  )
}
...
function SavedToast({ show }: { show: boolean }) {
  if (!show) return null
  return (
    <div className="fixed bottom-6 right-6 z-[70] animate-in fade-in slide-in-from-bottom-4 rounded-sm border border-primary/30 bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground shadow-lg">
      Cambios guardados
    </div>
  )
}
...
/* ── Category Modal ── */
function CategoryModal({ category, onSave, onClose }: { category: Category | null; onSave: (d: { name: string; description: string; sort_order: number }) => void; onClose: () => void }) {
  ...
  return (
    <ModalShell title={category ? "Editar Categoria" : "Nueva Categoria"} onClose={onClose}>
      <form onSubmit={(e) => { e.preventDefault(); onSave({ name, description, sort_order: sortOrder }) }} className="flex flex-col gap-4">
        <Field label="Nombre" id="cat-name"><input id="cat-name" type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} required /></Field>
        <Field label="Descripcion" id="cat-desc"><textarea id="cat-desc" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} className={inputCls} /></Field>
        <Field label="Orden" id="cat-order"><input id="cat-order" type="number" min="1" value={sortOrder} onChange={(e) => setSortOrder(Number.parseInt(e.target.value, 10) || 1)} className={inputCls} /></Field>
        <div className="mt-2 flex gap-3">
          <button type="submit" className="flex-1 rounded-sm bg-primary py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">{category ? "Guardar" : "Crear"}</button>
          <button type="button" onClick={onClose} className="flex-1 rounded-sm border border-border bg-transparent py-2.5 text-sm font-medium text-foreground hover:bg-muted">Cancelar</button>
        </div>
      </form>
    </ModalShell>
  )
...
/* ── Product Modal ── */
function ProductModal({ product, categories, onSave, onClose }: { product: Product | null; categories: Category[]; onSave: (d: { name: string; description: string; price: number; category_id: string; image_url: string }) => void; onClose: () => void }) {
  ...
  return (
      <ModalShell title={product ? "Editar Producto" : "Nuevo Producto"} onClose={onClose}>
        <form onSubmit={(e) => { e.preventDefault(); onSave({ name, description, price: Number.parseFloat(price), category_id: categoryId, image_url: image }) }} className="flex flex-col gap-4">
          <Field label="Nombre" id="prod-name"><input id="prod-name" type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} required /></Field>
          <Field label="Descripcion" id="prod-desc"><textarea id="prod-desc" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} className={inputCls} /></Field>
          <Field label="Categoria" id="prod-cat">
            <select id="prod-cat" value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className={inputCls} required>
              {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
          </Field>
          <Field label="Precio ($)" id="prod-price"><input id="prod-price" type="number" step="0.01" min="0" value={price} onChange={(e) => setPrice(e.target.value)} className={inputCls} required /></Field>
          <AdminImageUpload
          ...
          />
          <div className="mt-2 flex gap-3">
            <button type="submit" className="flex-1 rounded-sm bg-primary py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">{product ? "Guardar" : "Agregar"}</button>
            <button type="button" onClick={onClose} className="flex-1 rounded-sm border border-border bg-transparent py-2.5 text-sm font-medium text-foreground hover:bg-muted">Cancelar</button>
          </div>
        </form>
      </ModalShell>
...
/* ── Provider Modal (QR) ── */
function ProviderModal({ provider, paymentMethods, onSave, onClose }: { provider: MobilePaymentProvider | null; paymentMethods: PaymentMethod[]; onSave: (d: { name: string; qr_code_url: string; payment_method_id: string }) => void; onClose: () => void }) {
  ...
  return (
      <ModalShell title={provider ? "Editar Proveedor QR" : "Nuevo Proveedor QR"} onClose={onClose}>
        <form onSubmit={(e) => { e.preventDefault(); if (hasMethods) onSave({ name, qr_code_url: qrUrl, payment_method_id: methodId }) }} className="flex flex-col gap-4">
          {!hasMethods && (
          ...
          <Field label="Nombre" id="prov-name"><input id="prov-name" type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} required /></Field>
          <Field label="Metodo de pago" id="prov-method">
            <select id="prov-method" value={methodId} onChange={(e) => setMethodId(e.target.value)} className={inputCls} required disabled={!hasMethods}>
          ...
          />
          <div className="mt-2 flex gap-3">
            <button type="submit" className="flex-1 rounded-sm bg-primary py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90" disabled={!hasMethods}>{provider ? "Guardar" : "Agregar"}</button>
            <button type="button" onClick={onClose} className="flex-1 rounded-sm border border-border bg-transparent py-2.5 text-sm font-medium text-foreground hover:bg-muted">Cancelar</button>
          </div>
        </form>
      </ModalShell>
...
function PaymentMethodModal({ method, onSave, onClose }: { method: PaymentMethod | null; onSave: (d: { code: string; name: string; description: string; sort_order: number; active: boolean }) => void; onClose: () => void }) {
    ...
    <ModalShell title={method ? "Editar Metodo de pago" : "Nuevo Metodo de pago"} onClose={onClose}>
      ...
      >
        <Field label="Tipo" id="pm-code">
          <select id="pm-code" value={code} onChange={(e) => handleCodeChange(e.target.value)} className={inputCls} required>
        ...
        <Field label="Nombre" id="pm-name">
          <input id="pm-name" type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} required />
        </Field>
        <Field label="Descripcion" id="pm-desc">
          <textarea id="pm-desc" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} className={inputCls} />
        </Field>
        <Field label="Orden" id="pm-order">
          <input id="pm-order" type="number" min="1" value={sortOrder} onChange={(e) => setSortOrder(Number.parseInt(e.target.value, 10) || 1)} className={inputCls} />
        </Field>
        ...
        <div className="mt-2 flex gap-3">
          <button type="submit" className="flex-1 rounded-sm bg-primary py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">{method ? "Guardar" : "Agregar"}</button>
          <button type="button" onClick={onClose} className="flex-1 rounded-sm border border-border bg-transparent py-2.5 text-sm font-medium text-foreground hover:bg-muted">Cancelar</button>
        </div>
      </form>
    </ModalShell>
...
export default function AdminDashboardPage() {
  ...
  const [providerModal, setProviderModal] = useState<{ open: boolean; editing: MobilePaymentProvider | null }>({ open: false, editing: null })
  const [paymentModal, setPaymentModal] = useState<{ open: boolean; editing: PaymentMethod | null }>({ open: false, editing: null })
  ...
  // ── Payment Method CRUD ──
  const savePaymentMethod = async (data: { code: string; name: string; description: string; sort_order: number; active: boolean }) => {
  ...
  const deletePaymentMethod = async (id: string) => {
    const result = await adminRpc<Array<{ success: boolean; message?: string }> | { success: boolean; message?: string }>("sp_delete_payment_method", {
  ...
  const addStat = () => {
    ...
    const next: AboutStat[] = [
      ...(about.stats ?? []),
      { id: createTempId(), florist_id: floristId, label: "", value: "", sort_order: (about.stats?.length ?? 0) + 1 },
    ]
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      {sidebarOpen && <div className="fixed inset-0 z-40 bg-foreground/30 lg:hidden" onClick={() => setSidebarOpen(false)} role="button" tabIndex={-1} aria-label="Cerrar menu" onKeyDown={() => {}} />}

      {/* Sidebar */}
      <aside className={`fixed left-0 top-0 z-50 flex h-full w-64 flex-col border-r border-border bg-card transition-transform lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between border-b border-border px-6 py-5">
          <Link href="/" className="flex flex-col">
            <span className="font-serif text-xl font-bold tracking-wide text-primary">{florist?.name ?? "Admin"}</span>
            <span className="text-xs text-muted-foreground">Administracion</span>
          </Link>
          <button className="text-muted-foreground lg:hidden" onClick={() => setSidebarOpen(false)} aria-label="Cerrar"><X className="h-5 w-5" /></button>
        </div>
        ...
        <div className="border-t border-border px-3 py-4">
          <button onClick={() => setShowPassModal(true)} className="mb-2 flex w-full items-center gap-3 rounded-sm px-4 py-2.5 text-sm text-foreground/70 hover:bg-muted">Cambiar contrasena</button>
          <button onClick={handleLogout} className="flex w-full items-center gap-3 rounded-sm px-4 py-2.5 text-sm text-foreground/70 hover:bg-muted"><LogOut className="h-4 w-4" />Cerrar Sesion</button>
        </div>
      </aside>
      ...
      <div className="flex flex-1 flex-col">
        <header className="flex items-center gap-4 border-b border-border bg-card px-6 py-4">
          <button className="text-foreground lg:hidden" onClick={() => setSidebarOpen(true)} aria-label="Abrir menu"><Menu className="h-5 w-5" /></button>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          {activeTab === "dashboard" && (
            <div className="flex flex-col gap-8">
              <div className="border border-accent/30 bg-accent/5 p-6">
                <h2 className="font-serif text-lg font-semibold text-foreground">Bienvenido al panel de {florist?.name}</h2>
                <p className="mt-2 text-sm text-muted-foreground">
                  Todos los datos se gestionan desde esta interfaz y se guardan directamente en la base de datos.
                </p>
              </div>
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {[
                  ...
                  <div key={card.label} className="flex items-start gap-4 border border-border bg-card p-6">
                    <div className="flex h-11 w-11 items-center justify-center rounded-sm bg-primary/10"><card.icon className="h-5 w-5 text-primary" /></div>
                    <div>
          ...
          {activeTab === "products" && (
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setProdModal({ open: true, editing: null })} className="inline-flex items-center gap-2 bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4" />Nuevo Producto</button>
              </div>
                  ...
                  <tbody>
                    {products.map((p) => (
                      <tr key={p.id} className="border-b border-border/50 last:border-0">
                        <td className="px-4 py-3"><Thumb src={p.image_url} alt={p.name} /></td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => setProdModal({ open: true, editing: p })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Editar"><Pencil className="h-4 w-4" /></button>
                            <button onClick={() => setConfirmDelete({ type: "product", id: p.id, name: p.name })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Eliminar"><Trash2 className="h-4 w-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {activeTab === "categories" && (
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setCatModal({ open: true, editing: null })} className="inline-flex items-center gap-2 bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4" />Nueva Categoria</button>
              </div>
              <div className="overflow-x-auto border border-border">
                <table className="w-full text-sm">
                  <thead className="border-b border-border bg-muted/50">
                  ...
                  <tbody>
                    {categories.map((c) => (
                      <tr key={c.id} className="border-b border-border/50 last:border-0">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => setCatModal({ open: true, editing: c })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Editar"><Pencil className="h-4 w-4" /></button>
                            <button onClick={() => setConfirmDelete({ type: "category", id: c.id, name: c.name })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Eliminar"><Trash2 className="h-4 w-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {activeTab === "about" && about && (
              <div className="flex flex-col gap-8">
                <div className="flex flex-col gap-6 border border-border bg-card p-8">
                  ...
                  <Field label="Titulo" id="about-title">
                    <input id="about-title" type="text" value={about.title ?? ""} onChange={(e) => setAbout({ ...about, title: e.target.value })} className={inputCls} />
                  </Field>
                  <AdminImageUpload
              ...
              <div className="flex flex-col gap-6 border border-border bg-card p-8">
                <h2 className="font-serif text-lg font-semibold text-foreground">Parrafos de la seccion</h2>
                {(about.paragraphs ?? []).length === 0 && (
                  <p className="text-sm text-muted-foreground">Agrega parrafos para mostrar en la seccion Nosotros.</p>
                )}
              ...
              <div className="flex flex-col gap-6 border border-border bg-card p-8">
                ...
                {(about.stats ?? []).length === 0 && (
                  <p className="text-sm text-muted-foreground">Agrega cifras o hitos que quieres destacar.</p>
                )}
                <div className="flex flex-col gap-4">
              ...
              <button onClick={saveAbout} className="self-start rounded-sm bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">Guardar cambios</button>
            </div>
          )}
          ...
          {activeTab === "contact" && (
            <div className="flex flex-col gap-8">
              ...
              <button onClick={saveContact} className="self-start rounded-sm bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">Guardar cambios</button>
            </div>
          )}
          ...
          {activeTab === "payments" && (
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setPaymentModal({ open: true, editing: null })} className="inline-flex items-center gap-2 bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4" />Nuevo metodo</button>
              </div>
              <div className="overflow-x-auto border border-border">
                <table className="w-full text-sm">
                  <thead className="border-b border-border bg-muted/50">
                  ...
                  <tbody>
                    {paymentMethods.map((m) => (
                      <tr key={m.id} className="border-b border-border/50 last:border-0">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => setPaymentModal({ open: true, editing: m })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Editar"><Pencil className="h-4 w-4" /></button>
                            <button onClick={() => setConfirmDelete({ type: "payment", id: m.id, name: m.name })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Eliminar"><Trash2 className="h-4 w-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button onClick={() => setProviderModal({ open: true, editing: null })} className="inline-flex items-center gap-2 bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4" />Nuevo QR</button>
              </div>
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {providers.map((prov) => (
                  <div key={prov.id} className="flex flex-col items-center gap-4 border border-border bg-card p-6">
                    {prov.qr_code_url ? (
                      <div className="relative aspect-square w-36 overflow-hidden border border-border bg-background">
                        <Image src={prov.qr_code_url} alt={`QR ${prov.name}`} fill className="object-contain p-2" />
                      </div>
                    ) : (
                      <div className="flex aspect-square w-36 items-center justify-center border border-border bg-background text-muted-foreground">Sin QR</div>
                    )}
                    <span className="text-sm font-semibold text-foreground">{prov.name}</span>
                    <div className="flex gap-2">
                      <button onClick={() => setProviderModal({ open: true, editing: prov })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Editar"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => setConfirmDelete({ type: "provider", id: prov.id, name: prov.name })} className="rounded-sm p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Eliminar"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* Modals */}
      {catModal.open && <CategoryModal category={catModal.editing} onSave={saveCategory} onClose={() => setCatModal({ open: false, editing: null })} />}
      {prodModal.open && <ProductModal product={prodModal.editing} categories={categories} onSave={saveProduct} onClose={() => setProdModal({ open: false, editing: null })} />}
      {providerModal.open && <ProviderModal provider={providerModal.editing} paymentMethods={paymentMethods.filter((m) => m.code?.toUpperCase() === "MOBILE_PAY")} onSave={saveProvider} onClose={() => setProviderModal({ open: false, editing: null })} />}
      {paymentModal.open && <PaymentMethodModal method={paymentModal.editing} onSave={savePaymentMethod} onClose={() => setPaymentModal({ open: false, editing: null })} />}
      {confirmDelete && (
        <ConfirmDialog
          ...
          onConfirm={() => {
            ...
            else if (confirmDelete.type === "provider") deleteProvider(confirmDelete.id)
            else deletePaymentMethod(confirmDelete.id)
          }}
        ...
        <ModalShell title="Cambiar contrasena" onClose={() => setShowPassModal(false)}>
          <form onSubmit={changePassword} className="flex flex-col gap-4">
            ...
            <Field label="Contrasena actual" id="old-pass">
              <input id="old-pass" type="password" value={oldPass} onChange={(e) => setOldPass(e.target.value)} className={inputCls} required />
            </Field>
            <Field label="Nueva contrasena" id="new-pass">
              <input id="new-pass" type="password" value={newPass} onChange={(e) => setNewPass(e.target.value)} className={inputCls} required />
            </Field>
            <div className="mt-2 flex gap-3">
              <button type="submit" className="flex-1 rounded-sm bg-primary py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">Guardar</button>
              <button type="button" onClick={() => setShowPassModal(false)} className="flex-1 rounded-sm border border-border bg-transparent py-2.5 text-sm font-medium text-foreground hover:bg-muted">Cancelar</button>
            </div>
          </form>
        </ModalShell>
      )}
