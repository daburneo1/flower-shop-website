...
export async function getHeroSlides(): Promise<HeroSlide[]> {
  ...
  const supabase = await createClient()
  const { data } = await supabase.rpc("sp_list_hero_slides", {
    p_florist_id: floristId,
  })
...
export async function getCategories(): Promise<Category[]> {
  ...
  const supabase = await createClient()
  const { data } = await supabase.rpc("sp_list_categories", {
    p_florist_id: floristId,
  })
...
export async function getProducts(): Promise<Product[]> {
  ...
  return getProductsByCategories(floristId, categories)
}
...
export async function getPaymentMethods(): Promise<PaymentMethod[]> {
  ...
  const [{ data: methods }, { data: providers }] = await Promise.all([
    supabase.rpc("sp_list_payment_methods", { p_florist_id: floristId }),
    supabase.rpc("sp_list_mobile_payment_providers", { p_florist_id: floristId }),
  ])

  const mappedMethods: PaymentMethod[] = (methods ?? []).map(
      (row: Record<string, unknown>) => mapPaymentMethodRow(row, floristId),
  )

  const mappedProviders: MobilePaymentProvider[] = (providers ?? []).map(
      (row: Record<string, unknown>) => mapMobilePaymentProviderRow(row, floristId),
  )
...
export async function getAboutSection(): Promise<AboutSection | null> {
  ...
  const [{ data: about }, { data: paragraphs }, { data: stats }] = await Promise.all([
    supabase.from("about_section").select("*").eq("florist_id", floristId).single(),
    supabase.from("about_paragraphs").select("*").eq("florist_id", floristId).order("sort_order"),
    supabase.from("about_stats").select("*").eq("florist_id", floristId).order("sort_order"),
  ])
  ...
  const mappedParagraphs = (paragraphs ?? []).map((row: Record<string, unknown>) => ({
    id: String(row.id),
  ...
  const mappedStats = (stats ?? []).map((row: Record<string, unknown>) => ({
    id: String(row.id),
    florist_id: floristId,
...
export async function getFloristFullData(): Promise<FloristFullData | null> {
  ...
  const [slides, categories, paymentMethods, about, contactMethods, socialLinks] =
      await Promise.all([
        getHeroSlides(),
        getCategories(),
        getPaymentMethods(),
  ...
  const floristId = getFloristId()
  const products = floristId ? await getProductsByCategories(floristId, categories) : []
